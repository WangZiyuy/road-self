"""Dynamic FIFO one-GPU-per-run launcher for frozen Stage S3E phases."""
...
def assert_frozen(expected_sha: str) -> None:
    ...
    if head != expected_sha or status:
        raise RuntimeError("Stage S3E requires a clean frozen checkout")


def worker_command(job: dict, code_sha: str) -> list[str]:
    command = [
        sys.executable, str(REPO_ROOT / "tools/seg_raster/train_stage_s3e.py"),
        "--run-code-sha", code_sha, "--run-key", job["run_key"],
        "--run-id", job["run_id"],
    ]
    if job.get("calibration_manifest"):
        command.extend(["--calibration-manifest", job["calibration_manifest"]])
    return command
...
def main() -> int:
    ...
    if plan.get("run_code_sha") != args.run_code_sha:
        raise RuntimeError("S3E phase plan SHA mismatch")
    ...
    phase_label = str(plan["phase"]).lower()
    inventory_path = args.output_root / (
        "stage_s3e_gpu_inventory_{}.json".format(phase_label))
    schedule_path = args.output_root / (
        "stage_s3e_gpu_schedule_{}.json".format(phase_label))
    write_json(inventory_path, {
        "stage": "seg_raster_stage_s3e", "phase": plan["phase"],
    ...
    maximum = min(len(pool), max(1, int(os.environ.get(
        "S3_MAX_PARALLEL", str(len(plan["jobs"]))))))
    ...
    while queue or running:
        while queue and scheduler.available:
            ...
            run_dir = REPO_ROOT / "data_self/stage_s3e_seg_raster" / job["run_id"]
            ...
            record = {
                "phase": plan["phase"], "run_key": job["run_key"],
                "run_id": job["run_id"], "physical_index": gpu_index,
                ...
                "stdout_path": "${S3E_RUN_ROOT}/" + job["run_id"] + "/stdout.log",
                "stderr_path": "${S3E_RUN_ROOT}/" + job["run_id"] + "/stderr.log",
            }
        ...
        for gpu_index, state in list(running.items()):
            ...
            summary_path = (REPO_ROOT / "data_self/stage_s3e_seg_raster"
