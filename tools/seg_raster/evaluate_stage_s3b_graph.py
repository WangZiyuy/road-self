"""Evaluate one Stage S3B common-step checkpoint under uniform graph caps."""
...
from tools.seg_raster.audit_stage_s3a_graph import (
    DENSIFY, MAX_CONTROLS, RECT, SNAP, build_graph, connectivity,
    directional_apls, junction_metrics, parse_graph, raw_stats,
    select_postprocessed_graph, topology_metrics)
from utils.seg_raster.stage_s3 import load_stage_s3_config, sha256_file
from utils.seg_raster.stage_s3b import GRAPH_CAPS
...
def prepare_config(args, payload: dict) -> Path:
    ...
    base = deep_merge(defaults, load_stage_s3_config(
        REPO_ROOT / "configs/stage_s3b_common.yml"))
    config = deepcopy(base)
    raster = args.run_key != "R0"
    ...
    source_control = args.source_runtime / "controls" / (control or "aligned")
    raster_link = trajectory_dir / "xian.png"
    if not raster_link.exists():
        raster_link.symlink_to(source_control / "xian_0_0.png")
    config["TASK"] = "stage_s3b_{}_graph".format(args.run_key)
    config["TRAJ"] = {
    ...
    config["DIR"].update({
        ...
        "TEST_TRAJ_DIR": str(trajectory_dir), "TRAJ_DIR": str(source_control),
    ...
    config["TRAIN"].update({"NUM_TARGETS": 4, "IMG_SZ": 4096,
                            "WINDOW_SIZE": 256, "BACKBONE_PRETRAINED": False,
                            "DATA_PARALLEL": False, "MODEL": "origin"})
    config["TEST"].update({
        "CKPT": "selected", "CKPT_FILE": "selected.pth.tar", "GPU_ID": "0",
        "DATA_PARALLEL": False, "BATCH_SIZE_SEG": 32, "BATCH_SIZE_ANCHOR": 15,
        "TEST_IMG_SZ": 4096, "NUM_TILES": 1, "CROP_SZ": 256,
        "SAMPLE_STEP": 1, "SKIP_EMPTY_CROP": False, "WINDOW_SIZE": 256,
        "INFER_STEP": "start", "SINGLE_REGION": "xian",
        ...
        "BINARIZE_MAP": {"ROAD_SEG_THRESHOLE": args.shared_threshold,
                         "JUNC_MAX_REGION_AREA": 200,
                         "JUNC_SEG_THRESHOLE": args.shared_threshold,
...
def main() -> int:
    ...
    parser.add_argument("--run-key", choices=("R0", "R1", "R2", "R3"), required=True)
    parser.add_argument("--checkpoint", type=Path, required=True)
    parser.add_argument("--source-runtime", type=Path, required=True)
    parser.add_argument("--shared-threshold", type=float, required=True)
    ...
    if payload.get("kind") != "versioned_model_only":
        raise RuntimeError("graph evaluation requires common-step model-only checkpoint")
    args.output_dir.mkdir(parents=True, exist_ok=True)
    config_path = prepare_config(args, payload)
    ...
    status = ("RESOURCE_CAP_REACHED" if resource["status"] == "RESOURCE_CAP_REACHED"
              else "PASS")
    result = {
        "stage": "seg_raster_stage_s3b", "status": status,
        ...
        "shared_probability_threshold": args.shared_threshold,
        ...
        "gpu_name": torch.cuda.get_device_name(0), "started_at_utc": started_at,
        "graph_sha256": sha256_file(predicted_path),
        "graph_logical_path": "${S3B_REMOTE_OUTPUT}/graph/" + args.run_key
                              + "/xian.graph"}
