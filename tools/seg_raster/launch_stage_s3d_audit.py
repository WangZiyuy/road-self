"""Dynamic one-GPU launcher for S3D forensic and production preflight."""
...
def assert_frozen(expected_sha: str) -> None:
    ...
    if head != expected_sha or status:
        raise RuntimeError("Stage S3D audit requires a clean frozen checkout")


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--run-code-sha", required=True)
    parser.add_argument("--action", choices=("forensic", "preflight"),
                        required=True)
    ...
    inventory_path = args.output_root / ("gpu_inventory_" + args.action + ".json")
    schedule_path = args.output_root / ("gpu_schedule_" + args.action + ".json")
    args.output_root.mkdir(parents=True, exist_ok=True)
    write_json(inventory_path, {
        "stage": "seg_raster_stage_s3d", "phase": phase,
    ...
    gpu = max(eligible, key=lambda row: (
        ...
        -max(row["utilization_series_percent"]), str(row["uuid"])))
    logs = args.output_root / "logs"
    logs.mkdir(parents=True, exist_ok=True)
    stdout = (logs / (args.action + ".stdout.log")).open("w", encoding="utf-8")
    stderr = (logs / (args.action + ".stderr.log")).open("w", encoding="utf-8")
    environment = dict(os.environ)
    environment["CUDA_VISIBLE_DEVICES"] = str(gpu["index"])
    command = [
        sys.executable,
        str(REPO_ROOT / "tools/seg_raster/audit_stage_s3d_remote.py"),
    ...
    while process.poll() is None:
        ...
        try:
            ...
            record["post_launch_contention"]["last_query_error"] = type(error).__name__
    stdout.close()
    stderr.close()
    record["exit_code"] = int(process.returncode)
    record["end_time"] = utc_now()
    result_path = args.output_root / (
        "stage_s3d_remote_preflight.json" if args.action == "preflight"
        else "stage_s3d_input_swap_matrix.json")
    if result_path.is_file():
        result = json.loads(result_path.read_text(encoding="utf-8"))
        record["peak_allocated_memory_mb"] = result.get("peak_allocated_memory_mb")
        record["peak_reserved_memory_mb"] = result.get("peak_reserved_memory_mb")
